...
<tr class="woocommerce-shipping-totals shipping">
	...
	<td data-title="<?php echo esc_attr( $package_name ); ?>">
		<?php if ( isset( $available_methods ) && is_array( $available_methods ) ) : ?>
			<ul id="shipping_method" class="woocommerce-shipping-methods shipping-price-info">
				<?php foreach ( $available_methods as $method ) : ?>
					<li>
						...
						printf( '<label for="shipping_method_%1$s_%2$s">%3$s</label>', $index, esc_attr( sanitize_title( $method->id ) ), preg_replace( '/(.*?):/', '<span class="shipping-price-info__name">$1:</span>', wc_cart_totals_shipping_method_label( $method ) ) ); // WPCS: XSS ok.
